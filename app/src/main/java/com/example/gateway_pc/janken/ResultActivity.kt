package com.example.gateway_pc.janken
...
class ResultActivity : AppCompatActivity() {
    ...
    private fun saveData(myHand: Int, comHand: Int, gameResult: Int) {
        ...
        val lastComHand = pref.getInt("GAME_RESULT", -1)
        ...
        editor.putInt("GAME_COUNT", gameCount + 1)
                .putInt("WINNING_STREAK_COUNT",
                        if (lastGameResult == 2 && gameResult == 2)
                            ...
                            0)
                .putInt("LAST _MY_HAND", myHand)
                .putInt("LAST_COM_HAND", comHand)
                .putInt("BEFORE_LAST_COMHAND", lastComHand)
    ...
    private fun getHand(): Int {
        ...
        val beforeLastComHand = pref.getInt("BEFORE_LAST_COMHAND", 0)
        val gameResult = pref.getInt("GAME_RESULT", -1)

        if (gameCount == 1) {
            if (gameResult == 2) {
                //前回の勝負が一回目で、コンピュータが買った場合、
                //コンピュータは次に出す手を変える
